...
10. **Catalogue commun = RÉSERVE, pas source vive** (ADR 0009). `site_id NULL` = catalogue du siège (modèles d'équipements/d'opérations, gammes-templates, modèles de DI) ; `site_id` renseigné = ce qui vit sur un site. Un écran de SITE ne propose **que** ce qui est sur le site — le commun ne s'y montre jamais (règle portée par les **queries**, pas par les composants : `modelesEquipementsQueries.list`, `modelesOperationsQueries.liables`, `demandesQueries.modelesDi.list`, `gammesQueries.list`). Pour l'utiliser, un site en **installe une copie** depuis la Bibliothèque (« Importer depuis le commun » → RPC `copier_modele_equipement` / `copier_modele_operation` / `copier_gamme`, ouvertes aux rôles métier ayant accès au site) ; la copie est indépendante. Garde-fous base : `check_categorie_modele` (une sous-catégorie de parc ne pointe qu'un modèle de SON site) et `check_gamme_modele_meme_perimetre` (113).
