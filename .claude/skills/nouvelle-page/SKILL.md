...
- **DocumentsTab** (pièces jointes) : `<DocumentsTab liaison="documents_<entite>" parentColumn="<entite>_id" parentId={item.id} uploadOpen onUploadOpenChange uploadInitialFiles uploadDefaultTypeNom acceptedMimes? className? />`. Glisser-déposer pleine page via `useFileDrop({ enabled: canManage, onFiles })` (depuis **`@/hooks/use-file-drop`**) + brique **`FileDropOverlay`** (`<FileDropOverlay show={dragging} />`, voile sobre) sur une zone `relative flex-1`. `className="min-h-0 flex-1"` dans une zone `flex flex-col` bornée → l'état vide « Aucun document » se centre. Formats par défaut = **PDF + toute image** (les images sont converties en **WebP compressé** à l'upload via `imageToWebp`, un WebP déjà optimal restant intact) ; `MIME_PDF` (PDF seul) vient de `@/features/documents/upload`. (Doctrine upload 3 étapes encapsulée par le composant.)
