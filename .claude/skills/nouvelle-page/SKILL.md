...
À la fin **toujours** : **`npm run verify`** (typecheck + lint + **format** + **tests**) puis `npm run build` (qui régénère `routeTree.gen.ts`). L'ancienne gate omettait format et tests — d'où 170 fichiers dérivés hors format et 2 tests rouges restés invisibles.
...
**Listes** — `ListRow` (ligne générique : `media`/`title`/`subtitle`/`badges`/`meta`/`mobileMeta`/`actions`/`onClick`/`size`) · `RowMediaIcon` (icône pour entité **sans** image — passer le **composant** : `icon={HardHat}`) · `OperationRow` · `ListRowSkeletons` · `CardSkeletons` (grilles de cartes) — `@/components/common/`. Entité **avec** image → `MiniatureThumb` (`@/features/miniatures/components/miniature-thumb`) en `media`. **Carte d'en-tête d'une fiche détail** (vignette carrée + grille d'infos compacte, hauteur `h-20`) → `DetailHeaderCard` (`thumbnail` + `fields: ({ label, value } | null)[]` — un `null` = cellule vide qui préserve l'alignement ; `columns` 2 ou 3 ; partagée OT/gamme) — `@/components/common/`. **Liste de pièces jointes** (rangée + aperçu + menu Télécharger/**Modifier**/Détacher/Supprimer + confirmations, warning unique) → `DocumentsListe` (`docs`/`canEdit?`/`canDelete?`/`onDelete?`/`canDetach?`/`onDetach?`/`badges?`/`mobileMeta?`/`onPreview?` ; porte `useConfirmDelete` + toasts ; `canEdit` monte la modale d'édition nom+type AUTO-SUFFISante `DocumentEditDialog` — aucun câblage côté hôte) — `@/components/common/`.
...
**Dialogs & champs** — `DialogShell` (coquille « 3 zones » en-tête fixe / corps défilant / pied fixe, `max-h-85vh` ; variantes `size` (sm→full) / `padded` (corps plein cadre : aperçu, canvas) / `headerAction` / `headerSeparator` ; **base commune de TOUTES les modales** — `FormDialog`/`ConfirmDialog`/`ChecklistDialog`/`ExporterVersSiteDialog` + les cas plein cadre (aperçu document, sunburst, recadrage, picker) — la réutiliser pour tout nouveau dialog au lieu de recopier les classes) · `FormDialog` (coquille de tout dialog formulaire, bâtie sur `DialogShell` ; `<form>` react-hook-form, cf. §4) · `ConfirmDialog` (action ponctuelle/réversible) · `ConfirmDeleteDialog` (suppression définitive impact-aware) · `ConfirmDeleteCategorieDialog` (`@/features/categories/components/…`, suppression de catégorie : porte `useDeleteCategorie` + toast + blocage « non vide » via `enfants: { sousCategories, contenus, labelContenu }`) · `MotifDialog` (textarea obligatoire → action ; props `label`/`rows`/`destructive` ; ex. clôture avec compte-rendu, refus, annulation) · `ChecklistDialog` (+ `CheckRow` sur `Checkbox` Radix : multi-sélection cochable + recherche ; `items: {id, titre, sousTitre?, badge?}[]`/`initialSelected`/`onSubmit(ids)`) · `ExporterVersSiteDialog` · **Champs react-hook-form** dans **`@/components/common/fields/`** (`control`+`name`, erreur via `FormMessage`) : `TextField`/`TextareaField`/`DescriptionField`/`SelectField` (Select **Radix**, API `options`)/`NumberField`/`CheckboxField`/`SwitchField`/`RadioField`/`PorteeField` (« Portée » Commun/site, Bibliothèque)/`IdentiteFields` (nom+description+miniature) · `LocalEquipementFields` (`@/features/equipements/components/…`, cascade Localisation → Équipement du local, API `value`/`onChange` à ponter via `useWatch`/`setValue`) · `ChampValeurInput`/`ChampsListEditor` (caractéristiques dynamiques, `value`/`onChange`) · `MiniatureField` (`@/features/miniatures/components/miniature-field`, upload de vignette) — `@/components/common/` sauf indiqué. Primitives shadcn/Radix sous les champs : `ui/{form,checkbox,radio-group,switch,select-dropdown,separator,tabs,input,textarea,label}`.

**Statut & documents** — `StatusStepper` (frise, states calculés en amont) · `ProgressBar` (barre de progression neutre `value` 0..1 + `tone` sémantique, `role="progressbar"` ; aucune logique — l'appelant fournit valeur et tonalité ; ex. progression d'un contrat) · `DocumentsTab` (pièces jointes, doctrine 3 étapes ; `canAttach?` pour verrouiller l'ajout quand l'entité hôte est en lecture seule, ex. contrat résilié/expiré) · `FileDropField` (zone de dépôt dans un dialog) · `FileDropOverlay` (surcouche d'invite « Déposez pour rattacher » sur une zone `relative`, pilotée par `dragging` de `useFileDrop` — glisser-déposer pleine page) · `iconeFormat`/`PdfFileIcon` (icône selon le **format/MIME**, pas le type métier) — `@/components/common/`.

**Carte riche par entité** — quand une entité ne tient PAS sur une `ListRow` (statut détaillé + sous-statut, barre de progression, ligne d'infos adaptative, sous-blocs documents/historique), composer une carte dédiée sur `Card`/`CardContent` (pas `ListRow`) et **centraliser toute la logique dans `features/<x>/etat.ts`** (la carte ne calcule rien). Réf. : `ContratCard` (`src/features/prestataires/components/contrat-card.tsx`) — statut/progression/alerte via `etat.ts`, actions en `TooltipIconButton`, zone documents/versions en `children`.
...
- **Tokens sémantiques** (`bg-primary`, `text-warning`, `text-destructive`, `text-success`…), **jamais** de couleur en dur. Deux emplois **distincts** de `destructive` : **teinté** (`StatusBadge`, `StatusTone`, cellules de planning — `bg-destructive/10`) = état **critique** légitime (OT en retard, CapEx refusé, échéance dépassée) ; **solide** (`Badge`/`Button variant="destructive"`) = **action destructrice** uniquement. `warning` = état défavorable non critique. Les tonalités d'état viennent de `StatusTone` (`common/status-badge.tsx`), jamais d'une variante de `Badge`.
...
- `npm run verify` (typecheck + lint + format + tests) et `npm run build` au vert.
