...
import { useGammeBadges } from '../use-gamme-badges'
...
export function GammesExplorer({ siteId }: { siteId: string }) {
  ...
  // Badges de statut AGRÉGÉS du palier (calcul des OT à charger + agrégation par
  // gamme/catégorie/sous-catégorie), extraits dans useGammeBadges. Comportement
  // identique : les badges suivent clôtures/réouvertures (realtime OT interne au hook).
  const { otsParGamme, otsBadgesIndispo, statutParNode } = useGammeBadges({
    siteId,
    depth,
    gammeCats,
    gammes,
